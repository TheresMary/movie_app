...
const TabIcon = ({ focused, icon, text }: any) => {
  if (focused) {
    return (
      <View
        className={`flex-row flex-1 w-full justify-center items-center gap-1 mt-1 min-w-[100px] min-h-[45px] rounded-3xl overflow-hidden bg-primary-gradient`}
      >
    ...
    <Tabs screenOptions={{ 
      ...
      <Tabs.Screen
        ...
        options={{
          ...
          headerShown: false,
          tabBarIcon: ({ focused }) => (
            <TabIcon focused={focused} icon={icons.save} text="Bookmark" />
          ),
        }}
      />
      <Tabs.Screen
